...
{
	std::string sql = "SELECT * FROM student_faive_class";
	if(mysql_query(mysql_base, sql.c_str()) == true){
...
{
	std::string sql = "INSERT INTO student_faive_class (name, age, school, hobby) values ";
	sql += "(";
	sql +=data;
	sql +=");";
...
int main()
{
	...
	const std::string _passwd = "";
	const std::string _db   = "student_dd";
	const std::string data  = "'sss', 24, 'xgd', 'sleep'";
